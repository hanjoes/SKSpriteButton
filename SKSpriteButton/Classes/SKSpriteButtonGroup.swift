...
public class SKSpriteKitButtonGroup {
    ...
    private var eventHandler: SKSpriteButtonEventListener!

    init() {
        eventHandler = SKSpriteButtonEventListener(handler: self.toggleAction, forEvent: .touchesToggledOn)
    }
    ...
    public func removeButton(_ button:SKSpriteButton) {
        if toggleGroup.contains(button) {
            button.removeEventListener(eventHandler)
